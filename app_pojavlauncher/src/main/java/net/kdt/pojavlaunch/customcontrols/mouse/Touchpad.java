...
public class Touchpad extends View implements GrabListener, AbstractTouchpad {
    ...
    private void init() {
        if(LauncherPreferences.PREF_CUSTOM_MOUSE_PATH != null && !LauncherPreferences.PREF_CUSTOM_MOUSE_PATH.isEmpty()) {
            File customCursorFile = new File(LauncherPreferences.PREF_CUSTOM_MOUSE_PATH);
            if(customCursorFile.isFile()) {
                mMouseCursorDrawable = new BitmapDrawable(getResources(), BitmapFactory.decodeFile(customCursorFile.getAbsolutePath()));
            }
        }
